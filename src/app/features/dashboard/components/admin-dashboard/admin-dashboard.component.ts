...
export class AdminDashboardComponent implements OnInit {
  ...
  totalProjects = computed(() => this.dashboardData()?.totalProjects || 0);
  activeProjects = computed(() => this.projectService.projects().filter(p => p.status === 'ACTIVE').length);
  
  totalTasks = computed(() => this.dashboardData()?.totalTasks || 0);
  completedTasks = computed(() => this.dashboardData()?.completedTasks || 0);
  activeTasks = computed(() => (this.dashboardData()?.inProgressTasks || 0) + (this.dashboardData()?.todoTasks || 0));
  
  overdueTasks = computed(() => this.dashboardData()?.overdueTasks || 0);
  budgetUsedPercentage = computed(() => this.dashboardData()?.budgetUsedPercentage || 0);
  userRole = computed(() => this.dashboardData()?.role || '');
